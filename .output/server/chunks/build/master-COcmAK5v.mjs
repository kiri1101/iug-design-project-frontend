...
import script$2 from './index-CIYZaRL-.mjs';
import script$1 from './index-CwRA478j.mjs';
import script from './index-_KFYCzBp.mjs';
import { ref, computed, mergeProps, withCtx, createVNode, unref, toDisplayString, withModifiers, createBlock, createCommentVNode, createTextVNode, openBlock, isRef, Fragment, renderList, withDirectives, useSSRContext } from 'vue';
import { ssrRenderAttrs, ssrRenderComponent, ssrInterpolate, ssrRenderStyle, ssrRenderList, ssrRenderClass, ssrGetDirectiveProps, ssrRenderSlot } from 'vue/server-renderer';
...
const _sfc_main = {
  ...
  setup(__props) {
    ...
    return (_ctx, _push, _parent, _attrs) => {
      ...
      const _component_Drawer = script$1;
      const _component_SpeedDial = script$2;
      ...
      _push(`<div class="flex flex-row items-center space-x-2"><div class="relative cursor-pointer hover:scale-110 transition duration-200 ease-linear"><i class="pi pi-bell"></i><span class="absolute top-0 right-0 flex size-2"><span class="absolute inline-flex h-full w-full animate-ping rounded-full bg-orange-400 opacity-75"></span><span class="relative inline-flex size-2 rounded-full bg-orange-500"></span></span></div><ul><li class="text-sm"><img src="https://picsum.photos/id/237/200/300" class="object-cover object-center rounded-full cursor-pointer size-7" alt="Profile picture">`);
      _push(ssrRenderComponent(_component_Popover, {
        ...
        default: withCtx((_, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="w-52"${_scopeId}><ul class="space-y-1"${_scopeId}><li class="flex items-center p-2 border-b border-gray-300 capitalize"${_scopeId}><i class="mr-2 pi pi-user"${_scopeId}></i><div${_scopeId}><h5${_scopeId}>${ssrInterpolate(unref(authUser)?.user.fullName)}</h5><h5 class="text-xs"${_scopeId}>${ssrInterpolate(unref(authUser)?.user.role[0]?.name)}</h5></div></li><li class="flex items-center justify-between p-2 text-gray-900 transition duration-150 ease-linear rounded cursor-pointer hover:bg-emerald-400 hover:text-gray-100"${_scopeId}><span${_scopeId}><i class="mr-1 pi pi-sign-out"${_scopeId}></i> Logout </span>`);
            if (unref(isLoading)) {
            ...
            return [
              createVNode("div", { class: "w-52" }, [
                createVNode("ul", { class: "space-y-1" }, [
                  createVNode("li", { class: "flex items-center p-2 border-b border-gray-300 capitalize" }, [
                    createVNode("i", { class: "mr-2 pi pi-user" }),
                    createVNode("div", null, [
                      createVNode("h5", null, toDisplayString(unref(authUser)?.user.fullName), 1),
                      createVNode("h5", { class: "text-xs" }, toDisplayString(unref(authUser)?.user.role[0]?.name), 1)
                    ])
                  ]),
                  createVNode("li", {
      ...
      _push(ssrRenderComponent(_component_Drawer, {
        visible: unref(showNotifyPanel),
        "onUpdate:visible": ($event) => isRef(showNotifyPanel) ? showNotifyPanel.value = $event : null,
        header: "Drawer"
      }, {
        container: withCtx(({ closeCallback }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="font-mono space-y-5"${_scopeId}><div class="flex flex-row justify-between items-center border-b border-gray-300 p-3"${_scopeId}><h2 class="text-2xl font-bold"${_scopeId}>Notifications</h2><i class="pi pi-times cursor-pointer hover:bg-gray-900 hover:text-white p-1 rounded transition duration-200 ease-linear" style="${ssrRenderStyle({ "font-size": "0.8rem" })}"${_scopeId}></i></div><div class="p-3 h-[calc(100dvh-5.5rem)] overflow-y-auto"${_scopeId}><ul class="space-y-3"${_scopeId}><!--[-->`);
            ssrRenderList(unref(notifyStore).list, (message) => {
              _push2(`<li class="text-xs bg-gray-700 text-gray-100 p-2 rounded"${_scopeId}>${ssrInterpolate(message.content)}</li>`);
            });
            _push2(`<!--]--></ul></div></div>`);
          } else {
            return [
              createVNode("div", { class: "font-mono space-y-5" }, [
                createVNode("div", { class: "flex flex-row justify-between items-center border-b border-gray-300 p-3" }, [
                  createVNode("h2", { class: "text-2xl font-bold" }, "Notifications"),
                  createVNode("i", {
                ...
                createVNode("div", { class: "p-3 h-[calc(100dvh-5.5rem)] overflow-y-auto" }, [
                  createVNode("ul", { class: "space-y-3" }, [
                    (openBlock(true), createBlock(Fragment, null, renderList(unref(notifyStore).list, (message) => {
                      return openBlock(), createBlock("li", {
                        key: message.id,
                        class: "text-xs bg-gray-700 text-gray-100 p-2 rounded"
                      }, toDisplayString(message.content), 1);
                    }), 128))
                  ])
                ])
              ])
      ...
      _push(`<section class="flex flex-row h-full"><span class="block md:hidden">`);
      _push(ssrRenderComponent(_component_SpeedDial, {
        ...
        item: withCtx(({ item, toggleCallback }, _push2, _parent2, _scopeId) => {
          if (_push2) {
            _push2(`<div class="relative"${_scopeId}><button${ssrRenderAttrs(mergeProps({
              type: "button",
            ...
            }, ssrGetDirectiveProps(_ctx, _directive_tooltip, `${item.label}`)))}${_scopeId}><i class="${ssrRenderClass(item.icon)}" style="${ssrRenderStyle({ "font-size": "0.8rem" })}"${_scopeId}></i></button><span class="absolute top-1.5 right-8 bg-white border border-gray-500 rounded-xs px-1 text-xs shadow"${_scopeId}>${ssrInterpolate(item.label)}</span></div>`);
          } else {
            return [
              createVNode("div", { class: "relative" }, [
                ...
                createVNode("span", { class: "absolute top-1.5 right-8 bg-white border border-gray-500 rounded-xs px-1 text-xs shadow" }, toDisplayString(item.label), 1)
              ])
            ];
          }
        }),
...
//# sourceMappingURL=master-COcmAK5v.mjs.map
